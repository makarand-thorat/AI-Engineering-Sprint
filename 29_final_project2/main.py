...
from agent import app as agent_graph,evaluate_output
import uvicorn
...
@app.post("/evaluate")
async def get_evaluation(request: EvalRequest):
    print(f"\n--- API CALL: /evaluate ---")
    print(f"Received content: {request.content[:50]}...") # Print first 50 chars
    
    metrics = evaluate_output(request.content)
    
    print(f"Returning to UI: {metrics}")
    return metrics
